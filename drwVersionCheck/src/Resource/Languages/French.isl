...
InvalidParameter=Un paramètre non valide a ét?pass??la ligne de commande :%n%n%1
SetupAlreadyRunning=L'assistant d'installation est déj?en cours d'exécution.
...
OnlyOnTheseArchitectures=Ce programme ne peut être install?que sur des versions de Windows qui supportent ces architectures : %n%n%1
...
WinVersionTooHighError=Ce programme ne peut pas être install?sous %1 version %2 ou supérieure.
...
SetupAppRunningError=L'assistant d'installation a détect?que %1 est actuellement en cours d'exécution.%n%nVeuillez fermer toutes les instances de cette application puis cliquer sur OK pour continuer, ou bien cliquer sur Annuler pour abandonner l'installation.
UninstallAppRunningError=La procédure de désinstallation a détect?que %1 est actuellement en cours d'exécution.%n%nVeuillez fermer toutes les instances de cette application  puis cliquer sur OK pour continuer, ou bien cliquer sur Annuler pour abandonner la désinstallation.
...
ExitSetupMessage=L'installation n'est pas terminée. Si vous abandonnez maintenant, le programme ne sera pas install?%n%nVous devrez relancer cet assistant pour finir l'installation.%n%nVoulez-vous quand même quitter l'assistant d'installation ?
...
InfoBeforeClickLabel=Lorsque vous êtes prêt ?continuer, cliquez sur Suivant.
WizardInfoAfter=Information
InfoAfterLabel=Les informations suivantes sont importantes. Veuillez les lire avant de continuer.
InfoAfterClickLabel=Lorsque vous êtes prêt ?continuer, cliquez sur Suivant.
...
SelectDirDesc=O?[name] doit-il être install??
...
InvalidDrive=L'unit?ou l'emplacement réseau que vous avez sélectionn?n'existe pas ou n'est pas accessible. Veuillez choisir une autre destination.
DiskSpaceWarningTitle=Espace disponible insuffisant
DiskSpaceWarning=L'assistant a besoin d'au moins %1 Ko d'espace disponible pour effectuer l'installation, mais l'unit?que vous avez sélectionnée ne dispose que de %2 Ko d'espace disponible.%n%nSouhaitez-vous continuer malgr?tout ?
...
DirExists=Le dossier :%n%n%1%n%nexiste déj? Souhaitez-vous installer dans ce dossier malgr?tout ?
...
NoUninstallWarning=L'assistant d'installation a détect?que les composants suivants sont déj?installés sur votre système :%n%n%1%n%nDésélectionner ces composants ne les désinstallera pas pour autant.%n%nVoulez-vous continuer malgr?tout ?
ComponentSize1=%1 Ko
ComponentSize2=%1 Mo
...
SelectStartMenuFolderLabel3=L'assistant va créer les raccourcis du programme dans le dossier du menu Démarrer indiqu?ci-dessous.
...
WizardReady=Prêt ?installer
ReadyLabel1=L'assistant dispose ?présent de toutes les informations pour installer [name] sur votre ordinateur.
ReadyLabel2a=Cliquez sur Installer pour procéder ?l'installation ou sur Précédent pour revoir ou modifier une option d'installation.
ReadyLabel2b=Cliquez sur Installer pour procéder ?l'installation.
...
PreviousInstallNotCompleted=L'installation ou la suppression d'un programme précédent n'est pas totalement achevée. Veuillez redémarrer votre ordinateur pour achever cette installation ou suppression.%n%nUne fois votre ordinateur redémarr? veuillez relancer cet assistant pour reprendre l'installation de [name].
CannotContinue=L'assistant ne peut pas continuer. Veuillez cliquer sur Annuler pour abandonner l'installation.
ApplicationsFound=Les applications suivantes utilisent des fichiers qui doivent être mis ?jour par l'assistant. Il est recommand?d'autoriser l'assistant ?fermer ces applications automatiquement.
ApplicationsFound2=Les applications suivantes utilisent des fichiers qui doivent être mis ?jour par l'assistant. Il est recommand?d'autoriser l'assistant ?fermer ces applications automatiquement. Une fois l'installation terminée, l'assistant essaiera de relancer ces applications.
CloseApplications=&Arrêter les applications automatiquement
DontCloseApplications=&Ne pas arrêter les applications
ErrorCloseApplications=L'assistant d'installation n'a pas pu arrêter toutes les applications automatiquement. Nous vous recommandons de fermer toutes les applications qui utilisent des fichiers devant être mis ?jour par l'assistant d'installation avant de continuer.
...
FinishedLabel=L'assistant a termin?l'installation de [name] sur votre ordinateur. L'application peut être lancée ?l'aide des icônes créées sur le Bureau par l'installation.
...
SelectDiskLabel2=Veuillez insérer le disque %1 et cliquer sur OK.%n%nSi les fichiers de ce disque se trouvent ?un emplacement différent de celui indiqu?ci-dessous, veuillez saisir le chemin correspondant ou cliquez sur Parcourir.
PathLabel=&Chemin :
FileNotInDir2=Le fichier "%1" ne peut pas être trouv?dans "%2". Veuillez insérer le bon disque ou sélectionner un autre dossier.
...
ErrorFunctionFailedNoCode=%1 a échou?
ErrorFunctionFailed=%1 a échou?; code %2
ErrorFunctionFailedWithMessage=%1 a échou?; code %2.%n%3
...
ErrorRegCreateKey=Erreur lors de la création de la cl?de registre :%n%1\%2
ErrorRegWriteKey=Erreur lors de l'écriture de la cl?de registre :%n%1\%2

; *** INI errors
ErrorIniEntry=Erreur d'écriture d'une entrée dans le fichier INI "%1".

; *** File copying errors
FileAbortRetryIgnore=Cliquez sur Recommencer pour essayer ?nouveau, Ignorer pour passer ce fichier (déconseill?, ou Abandonner pour annuler l'installation.
FileAbortRetryIgnore2=Cliquez sur Recommencer pour essayer ?nouveau, Ignorer pour continuer malgr?tout (déconseill?, ou Abandonner pour annuler l'installation.
SourceIsCorrupted=Le fichier source est altér?
SourceDoesntExist=Le fichier source "%1" n'existe pas
ExistingFileReadOnly=Le fichier existant est protég?en lecture seule.%n%nCliquez sur Recommencer pour enlever la protection et essayer ?nouveau, Ignorer pour passer ce fichier, ou Abandonner pour annuler l'installation.
ErrorReadingExistingDest=Une erreur s'est produite lors de la tentative de lecture du fichier existant :
FileExists=Le fichier existe déj?%n%nSouhaitez-vous que l'installation le remplace ?
ExistingFileNewer=Le fichier existant est plus récent que celui que l'assistant essaie d'installer. Il est recommand?de conserver le fichier existant.%n%nSouhaitez-vous conserver le fichier existant ?
...
ErrorRestartReplace=Le marquage d'un fichier pour remplacement au redémarrage de l'ordinateur a échou?:
...
ErrorRegSvr32Failed=RegSvr32 a échou?et a retourn?le code d'erreur %1
...
UninstallUnknownEntry=Une entrée inconnue (%1) a ét?rencontrée dans le fichier journal de désinstallation
...
OnlyAdminCanUninstall=Ce programme ne peut être désinstall?que par un utilisateur disposant des droits d'administration.
UninstallStatusLabel=Veuillez patienter pendant que %1 est retir?de votre ordinateur.
UninstalledAll=%1 a ét?correctement désinstall?de cet ordinateur.
...
; Les messages personnalisés suivants ne sont pas utilis?par l'installation
...
AddonHostProgramNotFound=%1 n'a pas ét?trouv?dans le dossier que vous avez choisi.%n%nVoulez-vous continuer malgr?tout ?


;Ìí¼ÓµÄ¶àÓïÑÔ·­Òë

InstallTalkFrench=Vous avez install?une autre version de EaseUS Data Recovery Wizard, veuillez désinstaller pour continuer. Désinstallez maintenant?                  
EaseusComfirmContentFrench=Si possible, installez Data Recovery Wizard sur un lecteur autre que celui sur lequel vos fichiers ont ét?perdus afin d'éviter d'autres dommages ou la perte de données permanente.   
EaseusConfirmInstallFrench=Confirmer l'emplacement d'installation 
EaseusInstallSuccessFrench=L'installation a réussi   
EaseusRegisterInfoFrench=EaseUS Data Recovery Wizard 9.0 a ét?install?sur votre ordinateur avec succès.     
EaseusLaunchFrench=Faites Fonctionner EaseUS Data Recovery Wizard           
EaseusParticipateFrench=Participer au Programme d'amélioration de l'expérience de client     
SetupIsrunningFrench=En cours de fonctionnement du programme d’installation de EaseUS Data Recovery Wizard         
ButtonFinishTitleFrench=Termin?    
ButtonInstallTitleFrench=Installer     

EaseusAppName=EaseUS Data Recovery Wizard 9.0
EaseusPublish=easeUS
EaseusHelpURL=http://www.easeus.com/support.htm
EaseusURL=http://www.easeus.com
EaseusDirName=EaseUS\EaseUS Data Recovery Wizard 
EaseusCopyright=Copyright (c) 2004-2015 EaseUS.ALL RIGHTS RESERVED.
EaseusRun=Faites Fonctionner EaseUS Data Recovery Wizard
EaseusHelp=Aide de EaseUS Data Recovery Wizard
EaseusHelpComment=Vous guideront pour utiliser EASEUS Data Recovery Wizard
EaseusUninstallComment=Désinstaller EaseUS Data Recovery Wizard 9.0
LanguageType=French
LicenseAccepted = Accepter
Confirm = Confirmer
ProgramOnTheWeb = Visit EaseUS 