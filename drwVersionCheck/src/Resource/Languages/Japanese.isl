...
InstallTalkJapanese=貴方は既にEaseUS Data Recovery Wizardのもう一つバージョンをインストールしてあるため、先にアンインストールしてから継続して下さい。は即時にアンインストールする？                   
EaseusComfirmContentJapanese=復元したいデータのあるドライブにインストールすると、そのデータが上書きされる場合があります。リカバリーの成功率を維持するため、他のドライブにインストールしてください。  
EaseusConfirmInstallJapanese=インストール位置を確認する   
EaseusInstallSuccessJapanese=成功的にインストールした   
EaseusRegisterInfoJapanese=貴方の詳細情報を登記し、製品の情報、ニュース、提示及び贈り物を獲得する。     
EaseusLaunchJapanese=EaseUS Data Recovery Wizardを起動する           
EaseusParticipateJapanese=カスタマーエクスペリエンス向上プログラムに参加     
SetupIsrunningJapanese=EaseUS Data Recovery Wizard インストールプログラムは運行中である        
ButtonFinishTitleJapanese=完了     
ButtonInstallTitleJapanese=インストール     

EaseusAppName=EaseUS Data Recovery Wizard 8.0
EaseusPublish=EaseUS
EaseusHelpURL=http://jp.easeus.com/support.html
EaseusURL=http://jp.easeus.com
EaseusUpgrade=http://recovery.easeus.jp/download
EaseusDirName=EaseUS\EaseUS Data Recovery Wizard 
EaseusCopyright=Copyright (c) 2004-2015 EaseUS.ALL RIGHTS RESERVED.
EaseusRun=EaseUS Data Recovery Wizardを実行する
EaseusHelp=EaseUS Data Recovery Wizardヘルプ
EaseusHelpComment=EaseUS Data Recovery Wizardが使用できるように指導する
EaseusUninstallComment=EaseUS Data Recovery Wizard 0.0 をアンインストールする
LanguageType=Japanese
LicenseAccepted =同意
Confirm = 確認
ProgramOnTheWeb = EaseUSのホームページ
ProgramOnTheWebURL=http://recovery.easeus.jp/rd/home